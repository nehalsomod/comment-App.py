...
def deshboard():
    ...
    Month_Box['values'] = ('january','february','march','april','may','june','july','august','september','octomber','november','december')
    Month_Box.current(0)
    ...
    btn.place(x=550, y=230)

    con = pymysql.connect(host="localhost", user="root", password="", database="docterapp")
    cur = con.cursor()

    cur.execute("select * from use_information where username =" + user_name.get() + "'")
    rows = cur.fetchall()
...
def signup():
    ...
    #heading label
    heading = Label(winsignup, txt="signup", font='Verdana 20 bold')
    ...
    very_pass = Label(winsignup, text="password:", font='Verdana 10 bold')
    ...
    age = Entry(winsignup, width=40, textvariabel=age)
    age.place(x=200, y=193)

    Radio_button_male = ttk.Radiobutton(winsignup, text='Male', value="Male", variable=var).place(x=200, y=220)
    Radiobutton_female = ttk.Radiobutton(winsignup, text='Female', value="Female",variable=var).place(x=200, y=238)
